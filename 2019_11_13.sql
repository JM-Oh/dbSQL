...
--fk 제약에 의해 INSERT가 정상적으로 동작하지 못한다.
INSERT INTO emp_test VALUES (9998, 'sally', 2);

--무결성 제약에러 발생 시 뭘 해야 될까?
...
CREATE TABLE emp_test(
    empno NUMBER(4),
    ename VARCHAR2(10),
    --dept_test.deptno컬럼에 인덱스가 없기 때문에 정상적으로 fk 제약을 생성할 수 없다.
    deptno NUMBER(2) REFERENCES dept_test(deptno)
);
...
--FK 제약 옵션
--default : 데이터 입력/삭제 시 순차적으로 처리해줘야 fk제약을 위배하지 않는다.
--ON DELETE CASCADE : 부모 데이터 삭제 시 참조하는 자식 테이블 같이 삭제
--ON DELETE SET NULL : 부모 데이터 삭제 시 참조하는 자식 테이블 값 NULL로 설정
DROP TABLE emp_test;
CREATE TABLE emp_test(
    empno NUMBER(4) PRIMARY KEY,
    ename VARCHAR2(10),
...
--FK 제약 default 옵션 시에는 부모 테이블의 데이터를 삭제하기 전에 자식 테이블에서
--참조하는 데이터가 없어야 정상적으로 삭제가 가능했음
--ON DELETE CASCADE의 경우 부모 테이블 삭제 시 참조하는 자식 테이블의 데이터를 같이 삭제한다.
...
--FK 제약 default 옵션 시에는 부모 테이블의 데이터를 삭제하기 전에 자식 테이블에서
--참조하는 데이터가 없어야 정상적으로 삭제가 가능했음
--ON DELETE SET NULL의 경우 부모 테이블 삭제 시 참조하는 자식 테이블의 데이터의 참조 컬럼을 NULL로 설정한다.
--1.삭제 쿼리가 정상적으로 실행되는지?
...
commit;
--데이터타입을 변경하기 위해서는 컬럼 값이 비어있어야한다.
...
--FOREIGN KEY 추가
--실습
--1. dept테이블의 deptno 컬럼을 PRIMARY KEY 제약으로 테이블 변경
--ddl통해 생성
ALTER TABLE dept ADD CONSTRAINT pk_dept PRIMARY KEY (deptno);

--2. emp 테이블의 empno컬럼을 PRIMARY KEY 제약으로 테이블 변경
--ddl을 통해 생성
ALTER TABLE emp ADD CONSTRAINT pk_emp PRIMARY KEY (empno);

--3. emp 테이블의 deptno컬럼으로 dept테이블의 deptno 컬럼을 참조하는 fk제약을 테이블 변경
